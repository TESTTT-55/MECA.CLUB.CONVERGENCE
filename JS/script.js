...
(function () {
  ...
  function drawRockets(now) {
    ...
    for (let i = rockets.length - 1; i >= 0; i--) {
      ...
      if (r.goRight) {
        ...
        else if (r.x > canvas.width * 0.85)
          r.alpha = Math.max(r.alpha - 0.03, 0);
      } else {
        if (r.x > canvas.width * 0.9) r.alpha = Math.min(r.alpha + 0.05, 0.92);
        else if (r.x < canvas.width * 0.15)
          r.alpha = Math.max(r.alpha - 0.03, 0);
      }
      drawRollingRocket(r);
      r.x += r.vx;
      r.y += r.vy;
      if (r.x > canvas.width + 80 || r.x < -80 || r.alpha <= 0)
        rockets.splice(i, 1);
    }
  }
  ...
  function loop(ts) {
    ...
    stars.forEach((s) => {
      ...
      ctx.fillStyle =
        s.r > 1.2
          ? `rgba(232,213,160,${s.alpha})`
          : `rgba(200,210,240,${s.alpha})`;
      ctx.shadowBlur = s.r > 1.2 ? 8 : 2;
      ctx.shadowColor =
        s.r > 1.2 ? "rgba(200,168,78,0.6)" : "rgba(180,200,255,0.4)";
      ctx.fill();
      ctx.restore();
    });
...
const GOOGLE_SHEET_URL =
  "https://script.google.com/macros/s/AKfycby9a5C6Q0dsqm6bWHlXPzqo1EA4F8gsCVzgawxor_k1Esfo7SNG_bBQOSbDIIgUwHzV/exec";
...
obs.observe(document.getElementById("thankyou"), {
  attributes: true,
